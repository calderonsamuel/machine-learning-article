...
### GridSearchCV
Es una técnica esencial en el aprendizaje automático para optimizar los hiperparámetros de un modelo. Utiliza validación cruzada para evaluar exhaustivamente todas las combinaciones posibles de valores de hiperparámetros especificados. Por ejemplo, para un modelo SVM, se pueden explorar diferentes valores para los parámetros como C y el tipo de kernel. GridSearchCV entrena y evalúa el modelo con cada combinación, seleccionando automáticamente la configuración que maximiza una métrica de rendimiento definida, como la exactitud o el AUC. Aunque efectivo para mejorar el rendimiento del modelo, su implementación puede ser intensiva en recursos debido a la exhaustividad de la búsqueda, siendo crucial considerar la escalabilidad y el tiempo computacional requerido.
...
Los resultados de las métricas de evaluación para los diferentes modelos se presentan en la Tabla 1. 
